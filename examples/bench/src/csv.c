...
void samples_close(void) {
  ...
  for (u32 i = 0; i < g_n_entries; i++) {
    ...
    fwrite(g_index[i].interval,     1,           32, g_samples);
...
void csv_write_ops(
  ...
  fprintf(g_csv,
    ...
    "%.8e,%.8e,%.8e,%.8e,"
    ",,,,,\n",
    fmt_name, interval_label, op_name,
  ...
  strncpy(e->fmt, fmt_name, sizeof(e->fmt) - 1);
  e->fmt[sizeof(e->fmt)-1] = '\0';

  strncpy(e->interval, interval_label, sizeof(e->interval) - 1);
  e->interval[sizeof(e->interval)-1] = '\0';

  strncpy(e->op, op_name, sizeof(e->op) - 1);
