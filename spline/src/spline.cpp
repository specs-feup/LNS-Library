...
void greedy_spline_float2lns(std::vector<SplinePoint>& table, const size_t s_table, const i16 precision) {
  const i16
    start = 0,
    end   = ~0; // the closest to 1 we can get, 1 - 2^(-16)
...
void test_float2lns_table_sizes(const size_t s_max, const bool xf_mode, const bool lns16, const i16 precision) {
  ...
  for (size_t s_table = 2; s_table < s_max + 1; s_table++) {
    ...
    greedy_spline_float2lns(table, s_table, precision);
...
void test_lns2float_table_sizes(const size_t s_max, const bool xf_mode, const bool lns16, const i16 precision) {
  ...
  for (size_t s_table = 2; s_table < s_max + 1; s_table++) {
    std::vector<SplinePoint> table;
    greedy_spline_lns2float(table, s_table, precision);
...
void write_convert_table_binary(
  ...
  const bool lns16, const bool f2l,
  const i16 int_digits, const i16 precision
) {
  mkdir("lns_tables", 0755); // ignored if already exists
  ...
  const u32 s = (u32)spline.size();
  file.write(reinterpret_cast<const char*>(&s), sizeof(u32));
 
  // Convert tables are always stored in xf format (x, y pairs).
  if (lns16) {
...
f64 M_to_f64(const i16 a) {
  return static_cast<f64>(a) / (f64)(1 << 16);
}

i16 f64_to_M(const f64 a) {
  return static_cast<i16>(a * (f64)(1 << 16));
}
